...
try {
    if (
        ...
        $stmt = $conn->prepare("SELECT COUNT(*) as count FROM request WHERE request_id = :request_id and S_ID = :S_ID");
        $stmt->execute(array(':request_id' => $request_id,':S_ID' => $S_ID));
        ...
        $insertStmt = $conn->prepare("INSERT INTO request (
                    request_id, RE_how, RE_reason, RE_teacher_opinion, RE_teacherH_opinion,
                    RE_commentH, RE_comment, RE_status, S_ID, company_ID
                ) VALUES (
                    :request_id, :RE_how, :RE_reason, :RE_teacher_opinion, :RE_teacherH_opinion,
                    :RE_commentH, :RE_comment, :RE_status, :S_ID, :company_ID
                )");
